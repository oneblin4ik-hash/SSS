...
export async function sendCheckin(env, userId, day) {
  ...
  // Между уроком и чек-ином в норме двенадцать часов: утро и вечер. Но если
  // оба джоба пролежали просроченными — воркер не выкладывался, крон стоял, —
  // крон отправит их подряд, и «задание сделано?» придёт через секунду после
  // самого задания. Человек не успел не то что сделать, а дочитать. Такой
  // чек-ин просто пропускаем: одно невышедшее «как дела» ничего не стоит,
  // а вопрос невпопад показывает, что за тобой никто не следит.
  if (Date.now() - Date.parse(sent.sent_at) < 3 * 3600e3) return;
