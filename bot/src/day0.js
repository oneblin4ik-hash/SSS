...
export async function finish(env, chatId, uid, payload) {
  ...
  // Файл тоже предзапускный, пока продажа закрыта. Иначе человек читает
  // в сообщении «курс откроется такого-то числа», открывает приложенный файл —
  // а там ценник. Два разных сообщения в одну минуту, и верить нечему.
  await sendPdf(env, chatId, pre ? "offer-pre" : "offer",
