...
# в чём не бывало. Человек читал «курс откроется <дата>» и тут же
...
PRE_LAUNCH = None      # None — обычный оффер; строка с датой — предзапуск
...
# Порядок не случайный. Девятым кадром пришла Вика, и раскладка поменялась:
# при девяти снимках две плитки во всю ширину оставляли сироту в последнем
# ряду. Широкой осталась одна — Эльнур, он закрывает блок чемпионским
# кадром; остальные восемь встают четырьмя ровными рядами.
#
# Внутри рядов подобраны пары: сначала два длинных текста, потом два
# коротких — иначе ряд растягивается по высоте под самую многословную
# плитку, и рядом с ней зияет пустота. Женщин в наборе пять, мужчин
# четыре, поэтому в трёх рядах из четырёх стоят и те, и другие: строка
# читается обеими половинами аудитории, а не только своей.
RESULTS = [
    ("do-posle-zhenya", "Женя, 34 года", "Четыре месяца, 140 → 103 кг. "
     ...
     "на двенадцатый этаж без одышки.", False),
    ...
    # Единственный в блоке, у кого результат меряется не только весами.
    # Силовые в два-три раза — довод для того, кто боится «похудеть
    # и обессилеть», и такого довода больше нигде на странице нет.
    ("do-posle-german", "Герман", "Три месяца между кадрами. До этого шесть "
     "раз пробовал похудеть сам. Здесь — минус пятнадцать килограммов, "
     "а силовые во всех движениях выросли в два-три раза. Кадр «до» снят "
     "не в первый день: пару месяцев он стеснялся фотографироваться. "
     "Говорит, что главное изменение — уверенность в себе.", False),
    ("do-posle-70-62", "Виктория", "Три месяца, 70 → 62 кг. Работали онлайн, "
     "и это с травмами позвоночника.", False),
    # Единственный кадр, где весы почти не двигались. Без объяснения он
    # читается слабее, чем есть: человек смотрит и не понимает, что
    # изменилось.
    ("do-posle-vika", "Вика, 28 лет", "Четыре месяца. Офис, маленький ребёнок, "
     "десять килограммов после родов. Вес почти не сдвинулся — изменилось то, "
     "из чего он состоит: плюс тринадцать сантиметров в обхвате ягодиц, "
     "появилась талия, перестали болеть спина и шея.", False),
...
def price_html() -> str:
    ...
    if PRE_LAUNCH:
        return f"""<div class="price">
          ...
          <p class="sub">Сейчас его проходят четыре девушки, и я иду вместе
