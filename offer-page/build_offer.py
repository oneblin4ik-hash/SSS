...
ОТЗЫВЫ — ТОЖЕ ЗАГЛУШКИ, и это видно на слайде. Владелец разрешил придумать
...
REVIEWS = [
    ("Н", "Наталья", 41, "14 дней из 14, первая тренировка в жизни",
     "Всё откладывала: думала, сначала похудею, а спорт потом. Тут первые три "
     "дня вообще без тренировок, просто разбираешься со своим днём. На "
     "четвёртый сделала первую тренировку в жизни."),
    ("С", "Сергей", 36, "Вернулся после двух лет паузы",
     "Пробовал вернуться сам — на второй неделе потянул спину и снова всё "
     "бросил. Здесь первая неделя специально лёгкая, есть щадящий вариант. "
     "За две недели ничего ни разу не заболело."),
    ("И", "Ирина", 29, "Третий заход — первый, который не бросила",
     "Начинала раза четыре, дальше третьей недели не уходила. Разница "
     "оказалась в двенадцатом дне: там про то, что делать, когда всё пошло "
     "не по плану. Теперь просто продолжаю со следующего."),
]
...
def reviews_html() -> str:
    ...
    return f'{note}<ul class="rv-list">{cards}</ul>'
  ...
  <section class="slide" id="s3" aria-label="Слайд 3 из 3">
    <div class="col">
      ...
      <footer class="foot">
        ...
        бесплатно. Беру несколько разборов в неделю, живая очередь.</p>
