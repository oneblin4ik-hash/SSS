...
FIELDS = [
    ...
    ("zn", "акцент на зону", False),
    ("fn", "фигура сейчас", False),
    ("fg", "фигура цель", False),
    ("ex", "опыт", True),
    ("ls", "когда последний раз", False),
    ("dd", "что делал", False),
    ("mn", "минут на тренировку", False),
    ("fq", "раз в неделю", False),
    ("hl", "здоровье", False),
    ("t", "тип старта", True),
...
(function(){
  var FIELDS=[__ROWS__];
  var TYPES={never:"Чистый лист · опыта нет",
             quit:"Второй заход · опыт есть, пауза затянулась",
             onoff:"Рывками · начинаешь часто, не закрепляется"};
...
def main() -> None:
    ...
    html = html.replace(
        "<title>Тест: первые шаги к форме — Эдуард Серболин</title>",
