...
# Состав взят из атласа (`source/trenirovki-Serbolin.md`, §2): в прежней версии
# стояли отжимания от опоры и выпады назад, а видео с техникой на них нет.
#
# Две версии, м и ж, различаются одной строкой. Решение владельца: мужчинам
# ягодичный мостик не даём, его место занимают приседания у стены. Оговорка
# записана в §2 спеки: другого движения на заднюю цепь без оборудования
# в атласе нет, поэтому у мужчин дома она не закрыта.
_BASE = [
    ("Приседания без веса", "Садись до стула, не ниже"),
    ("Отжимания от лавки", "Чем выше опора, тем легче"),
    None,                                     # место под движение, которое разное
    ("Скручивания на пресс", "Меньше амплитуда, руки крест-накрест на груди"),
    ("Планка", "С колен, но спина прямая"),
]
_SEX_SLOT = {
    "zh": ("Ягодичный мостик", "Стопы ближе к тазу, если тянет колени"),
    "m": ("Приседания у стены", "Угол больше 90°, время меньше"),
}


def workout(sex: str) -> list[tuple[str, str]]:
    return [_SEX_SLOT[sex] if x is None else x for x in _BASE]
...
def day4(sex: str = "m") -> dict:
    body = f"""
{_table(["Упражнение", "1", "2", "3"], workout(sex), 3)}
<div class="note">В ячейки ставь количество повторов в каждом подходе.
  {"Приседания у стены и планку" if sex == "m" else "Планку"} записывай
  в секундах.</div>
...
"""
    return day_page(4, "Первая тренировка",
                    ...
                    "сесть на унитаз.", body, suffix=f"-{sex}")
...
def day9(sex: str = "m") -> dict:
    body = f"""
{_table(["Упражнение", "Д4", "Д9"], workout(sex), 2)}
...
"""
    return day_page(9, "Вторая тренировка + замер",
                    ...
                    "Этого хватит.", body, suffix=f"-{sex}")
...
def day11(sex: str = "m") -> dict:
    body = f"""
{_table(["Упражнение", "1", "2"], workout(sex), 2)}
<div class="note">Тот же комплекс, что в дни 4 и 9, только два подхода вместо
  трёх. Спокойный темп, без прыжков. Если совсем горит — один подход.</div>
...
"""
    return day_page(11, "Тренировка, когда некогда",
                    ...
                    "гости. На этот случай есть урезанная версия. Ничего нового "
                    "учить не надо: тот же комплекс, только в два подхода.", body,
                    suffix=f"-{sex}")
...
def day13(sex: str = "m") -> dict:
    body = f"""
{_table(["Упражнение", "Д4", "Д9", "Д13"], workout(sex), 3)}
...
{b.plate('Всё это остаётся с тобой, даже если сейчас закрыть бота. Дальше два '
         ...
         'путь на следующей странице: созвон 30 минут, бесплатно.</span>',
...
# пол — в базовом комплексе (дни 4, 9, 11, 13). Человек получает по одному
