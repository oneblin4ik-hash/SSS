...
export async function GET() {
  ...
  return NextResponse.json(audiences.map((a) => ({ id: a.id, name: a.name, count: a.count, status: a.status })));
}
...
export async function POST(req: NextRequest) {
  ...
  const account = await getAccountSession(user.id, parsed.data.accountId);
